...
8. [Несколько thunk в одном slice](#несколько-thunk-в-одном-slice)
9. [Типизация в RTK](#типизация-в-rtk)
10. [RTK vs базовый Redux — сравнение](#rtk-vs-базовый-redux--сравнение)
11. [Плюсы и минусы RTK](#плюсы-и-минусы-rtk)
12. [Когда что использовать](#когда-что-использовать)
13. [Пример для CRM + json-server](#пример-для-crm--json-server)
...
> **Одной фразой:** RTK делает то же самое, что базовый Redux, но в 2–3 раза меньше файлов и boilerplate (повторяющийся раз за разом код, который нельзя вынести в функцию).
...
## Несколько thunk в одном slice

**Короткий ответ:** несколько `createAsyncThunk` в одном slice — это нормально и так обычно и делают. Один slice = одна доменная сущность (students, blogs, auth), а внутри неё может быть сколько угодно async-операций.

### Как это работает

Каждый `createAsyncThunk` — **отдельный action** со своим type:

| Thunk           | Action types                                                 |
| --------------- | ------------------------------------------------------------ |
| `fetchStudents` | `students/fetchAll/pending`, `.../fulfilled`, `.../rejected` |
| `createStudent` | `students/create/pending`, `.../fulfilled`, `.../rejected`   |
| `deleteStudent` | `students/delete/pending`, `.../fulfilled`, `.../rejected`   |

В `extraReducers` вы просто добавляете `.addCase(...)` для каждого thunk — цепочкой через `builder`.

### Пример: полный CRUD в одном slice
...
const studentsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // create — только fulfilled, loading общий
...
const handleCreate = () => {
  dispatch(createStudent({ name: "Иван", email: "ivan@mail.ru" }));
};
...
### Общий `loading` vs отдельные флаги

**Один `loading`** — проще, но если одновременно идут fetch и create, флаг будет общим:

```ts
// fetchStudents.pending  → loading = true
// createStudent.pending  → loading = true (тот же флаг)
```

**Отдельные флаги** — когда нужно показывать разный UI (спиннер на списке, кнопка «Сохранить» и т.д.):
...
### Синхронные + async actions вместе

В одном slice могут жить и `reducers` (синхронные), и `extraReducers` (async):

| Где             | Что                                                                  |
| --------------- | -------------------------------------------------------------------- |
| `reducers`      | `clearError`, `resetFilters`, `setSelectedId` — мгновенные изменения |
| `extraReducers` | `fetchStudents`, `createStudent` — ответы от API                     |
...
### Сравнение с базовым Redux

| Базовый Redux                                             | Redux Toolkit                                                                  |
| --------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 3 константы на каждый thunk (`START`, `SUCCESS`, `ERROR`) | 1 `createAsyncThunk` — `pending/fulfilled/rejected` генерируются автоматически |
| 3 `case` в reducer на каждый thunk                        | `.addCase(thunk.pending, ...)` и т.д.                                          |
| Отдельный файл actions + reducer                          | Всё в одном `*Slice.ts`                                                        |

Логика та же, boilerplate меньше.

### Когда разделять на несколько slice

| Один slice                                      | Несколько slice                          |
| ----------------------------------------------- | ---------------------------------------- |
| `studentsSlice` — fetch, create, update, delete | `studentsSlice` + `studentsFiltersSlice` |
| Все операции над одной сущностью                | Разные домены (auth, students, lessons)  |

**Правило:** один slice = одна сущность/домен. Сколько угодно thunk внутри — нормально.

> **Пример из blog-application:** `fetchBlogs`, `deleteBlog`, `createBlog` логично держать в одном `blogsSlice.ts`, вместо отдельных `blogsActions.ts` + `blogsReducer.ts`.
...
| **Обучение**           | Понимаешь, _почему_ RTK устроен именно так   |
...
# 7. Несколько thunk в одном slice — нормально
# fetch, create, update, delete → один *Slice.ts, несколько createAsyncThunk
```
